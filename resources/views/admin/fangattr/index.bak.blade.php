...
@section('cnt')
    ...
    <div class="page-container">
        <form method="get" class="text-c"> 输入想要搜索的房源属性名称：
            <input type="text" class="input-text" style="width:250px" placeholder="房源属性"
                   value="{{ request()->get('name') }}" name="name" autocomplete="off">
            ...
            <table class="table table-border table-bordered table-hover table-bg table-sort">
                ...
                @foreach($data as $item)
                    <tr class="text-c">
                        ...
                        <td class="td-manage">
                            <a href="{{ route('admin.fangattr.edit',['id'=>$item['id']]) }}"
                               class="label label-secondary radius">修改</a>
                            {{--                            <a href="{{ route('admin.fangattr.edit',$item['id']) }}" class="label label-secondary radius">修改</a>--}}
                            <a data-href="{{ route('admin.fangattr.destroy',['id'=>$item['id']]) }}"
                               class="delbtn label label-warning radius">删除</a>
                        </td>
                    </tr>
                @endforeach
                </tbody>
