    ...
    <article class="page-container">
        ...
        <form action="{{ route('admin.fang.update',$fang) }}" method="post" class="form form-horizontal" id="fang-add">
                ...
                <div class="formControls col-xs-4 col-sm-4">
                    <select name="fang_province" style="width: 100px;" onchange="selectCity(this,'fang_city')">
                        ...
                        @foreach($cityData as $item)
                            <option @if($item->id == $fang->fang_province) selected
                    ...
                    <select name="fang_city" id="fang_city" style="width: 100px;"
                        ...
                        @foreach($currentCityData as $item)
                            <option @if($item->id == $fang->fang_city) selected
                                    @endif value="{{ $item->id }}">{{ $item->name }}</option>
                        @endforeach
                    </select>
                    <select name="fang_region" id="fang_region" style="width: 100px;">
                        @foreach($currentRegionData as $item)
                            <option @if($item->id == $fang->fang_region) selected
                                    @endif value="{{ $item->id }}">{{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="formControls col-xs-4 col-sm-5">
                    <input type="text" class="input-text" name="fang_addr" value="{{ $fang->fang_addr }}">
                </div>
            </div>
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>租金：</label>
                <div class="formControls col-xs-8 col-sm-9">
                    <input type="number" class="input-text" style="width: 200px;" name="fang_rent"
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <select name="fang_rent_type" style="width: 200px;">
                        @foreach($fang_rent_type_data as $item)
                            <option @if($item->id == $fang->fang_rent_type) selected
                                    @endif value="{{ $item->id }}">{{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <select name="fang_direction" style="width: 200px;">
                        @foreach($fang_direction_data as $item)
                            <option @if($item->id == $fang->fang_direction) selected
                                    @endif value="{{ $item->id }}">{{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <select name="fang_rent_class" style="width: 200px;">
                        @foreach($fang_rent_class_data as $item)
                            <option @if($item->id == $fang->fang_rent_class) selected
                                    @endif value="{{ $item->id }}">{{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <input type="number" class="input-text" name="fang_build_area" value="{{ $fang->fang_build_area }}"
                           style="width: 60px;">平米
                </div>
            </div>
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <input type="number" class="input-text" name="fang_using_area" value="{{$fang->fang_using_area}}"
                           style="width: 60px;">平米
                </div>
            </div>
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <input type="text" onfocus="WdatePicker({dateFmt:'yyyy'})" value="{{ $fang->fang_year }}"
                           name="fang_year" class="input-text Wdate" style="width:120px;">
                </div>
            </div>
            <div class="row cl">
                    ...
                    @foreach($fang_config_data as $item)
                        <label>
                            ...
                            <input @if(in_array($item->id,$fang->fang_conf)) checked @endif  type="checkbox"
                                   name="fang_config[]" value="{{ $item->id }}"/>
                            {{ $item->name }} &nbsp;&nbsp;
                        </label>
                    @endforeach
                </div>
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <select name="fang_owner" style="width: 200px;">
                        @foreach($ownerData as $item)
                            <option @if($item->id == $fang->fang_owner) selected
                                    @endif value="{{ $item->id }}">{{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
